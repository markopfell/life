...
\section{Skills and Exposure}
\begin{tabular}{ @{} >{\bfseries}l @{\hspace{6ex}} l }
Standards & CCSDS, DVB-S2, ITU, FCC\\
RF Tools & SDR, VNA, GNU Radio\\%, Antenna Hats\\
%Groundstation Network & KSAT Lite\\
General Software Tools & Python, Git*, Linux, Bash, AWS EC2\\%, Excel (Wizard)\\
Python Libraries & NumPy, Matplotlib, Scapy\\%,, Skimage SciPy\\
Networking & Ethernet, UDP/IP, Wireshark, iPerf\\ %Blacksky, Leostella public knowledge 
%Cloud & AWS EC2 \& S3\\ %Blacksky, Leostella public knowledge 
FPGA & Xilinx, PYNQ, Vivado\\
Significant Volcano Ascents &  Rainier, Baker, Adams\\%Stawamus Chief (Squamish Buttress face), 5 Gallon Buckets \\
...
\gray Job Title & \textbf{Senior RF and Telemetry Engineer}\\
\gray Employer & \textbf{Relativity} \hfill Long Beach, CA\\
\gray Period & \textbf{August 2025 -- Present}\\
%??,??? units of REL = $???/share private equity vesting: Year 1 = 5%, Year 2 = 15%, Year 2.5 = 20%, Year 3 = 20%, Year 3.5 = 20%, Year 4 = 20%.
%base $185,000 
%company_size=1,000
&
\vspace{-0.15 cm}
Networked, built, programmed, and tested RF HITL rack testing S-band telemetry-over-IP bit error rate from vehicle UDP/IP multicast network traffic to radiated PSK waveform received by the ground receiver.  Compared PTP versus NTP real world accuracies against telemetry timebase requirements.  Distributed stratum 1 time over NTP to key rack units.
\newline
\newline
Wrote, and tested high-level code generating HDL for a Xilinx FPGA-based pseudo-randomization algorithm proving basic spectral compliance.   
%Hands-on debugged a Xilinx FPGA-based software defined transmitter's digital modulator, frame synchronizer, and LDPC forward error correction encoder.  Corrected, committed, and re-built high-level code generating HDL reversing a subtle endianness swap, and errors in synchronization word construction.  Flashed bitstreams onto PCBAs, and validated system bit accuracy, and frame lock.
%Fixed major errors in high level HDL, and flashed working bitstreams onto PCBAs. 
\newline
%\newline
\end{tabularx}
...
\gray Period & \textbf{July 2024 -- August 2025}\\
...
&
\vspace{-0.15 cm}
%Developed Ka-band payload phased array system test infrastructure analyzing complex wireless links for mass production.
Implemented and ran, over the air Ka-band MIMO phased array system test UDP/IP throughput tests on Xilinx Versal FPGAs for a LEO constellation.  
%Developed a Python driver for Gateway LNB telemetry.
%Developed a Python driver capturing raw telemetry frames of RSSI on Gateway LNBs, and coupled RF transmit power from BUCs.  
%The Python driver could reboot individual gateway hardware and reverse faults. 
\newline
%Developed and tested Ka-band MIMO phased array antenna system test matrix in a spacecraft sized anechoic chamber.
%\newline
%\newline
\end{tabularx}
...
&
\vspace{-0.15 cm}
Built a large scale year-in-the-life Python link budget modeling satellite constellation average payload data throughput.  
%, and Python code for the processing pipeline.
\newline
\newline
%Lead NGSO imaging satellite constellation ITU, and FCC 312 Schedule S regulatory filings.  Ran RF analysis efforts with Python scripts, and ITU Spacecap.  
%Collaborated with orbital dynamics, and mechanical design experts to decompose legal wording into requirements for satellite architecture and material choices ensuring proper post mission disposal. 
%\newline
%\newline
%Created a weekly multi-system schedule for a years worth of work to get to launch.  This work plan burned down realistic technical risk of hardware and software through test and integration opportunities ultimately yielding a successful first RF contact with the space vehicle.
%Procured, set up, coded, documented, and maintained HITL (hardware-in-the-loop) testbed for S-band telemetry \& command megabit speed links. Hardware included an EDU space transceiver, ground COTS SDR, open-source spectrum analyzer, couplers, attenuators, and coax.  Defined and tested first contact ConOps. Architected and set up lab to cloud VPN network with a hardware firewall.  
%\newline
%\newline
%\newline
%\newline
%Continuously built consensus with the founders (CTO, CEO) on which space and ground communication business partnerships to pursue.  Developed 10 year communications technology roadmap. 
%Collaborated on product (satellite imagery) level vision.    
%\newline
%\newline
Created, evaluated, and built space-to-ground digital communications payload gigabit class link for VLEO visible and thermal imaging sensors. Developed the telemetry and command data chain from modulated waveform to frames, packets, and connections. Analyzed and tested with: software defined AD9361-based packet radios, GNU radio, and technical deep dives into CCSDS standards.
%Used model outputs to build a plan to increase throughput by 42\%.  
...
\gray Period & \textbf{September 2013 -- January 2018}\\
%total yearly compensation:$82k + 0k bonus + 0k stock, Senior Systems Engineer, company_size=4000
&
\vspace{-0.15 cm}
...
\gray Period & \textbf{2009 -- 2012}\\
\vspace{0.5 cm}
\end{tabularx}

\begin{tabularx}{0.97\linewidth}{>{\raggedleft\scshape}p{2cm}X}
\gray Certification & \textbf{Network Technician}\\
%\gray Focus & \textbf{Applied Electromagnetics}\\
\gray Organization & \textbf{Cisco} \\
\gray Period & \textbf{2024}\\
\vspace{0.5 cm}
\end{tabularx}

\begin{tabularx}{0.97\linewidth}{>{\raggedleft\scshape}p{2cm}X}
\gray Certification & \textbf{Apprentice Alpine Mountain Guide}\\
%\gray Focus & \textbf{Applied Electromagnetics}\\
\gray Organization & \textbf{American Mountain Guide Association} \\
\gray Period & \textbf{2023}\\
\vspace{0.5 cm}
\end{tabularx}
